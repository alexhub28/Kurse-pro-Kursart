...
function drawChart() {
  ...
  d3.csv("EAZ_Kurse_pro_Kursart_2025.csv").then(raw => {
    ...
    // catégories consécutives se touchaient presque).
    const rowHeight = 42;
    const margin = {
      top: 8,
    ...
    rowLabels.each(function (d) {
      ...
      group.append("tspan").attr("x", -10).attr("dy", "-0.5em").text(parts[0]);
      group.append("tspan").attr("x", -10).attr("dy", "1em").text(parts[1]);
      group.append("tspan").attr("x", -10).attr("dy", "1em").text(parts[2]);
    });
